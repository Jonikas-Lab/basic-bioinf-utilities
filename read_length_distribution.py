...
def print_lengths(readlen_counter,include_zeros=False):
    ...
    print "length\tread count"
    for l in lengths:
        print "%s\t%s"%(l,readlen_counter[l])
    print "Total %s reads"%sum(readlen_counter.values())

def read_length_distribution(infiles,include_zeros=False,verbose=False):
    ...
    for infile in infiles:
        ...
        try:    
            ...
            if verbose: print "File %s recognized as fasta, with %s total reads."%(infile,total_count)
            ...
            try:    
                ...
                if verbose: print "File %s recognized as fastq, with %s total reads."%(infile,total_count)
...
if __name__ == "__main__":
    ...
    parser.add_option('-v','--verbose', action='store_true', default=False, help="Print some info about each processed file (default %default).")
